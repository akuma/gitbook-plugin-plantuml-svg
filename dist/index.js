...
function getConfig(context, property, defaultValue) {
  var config = _.get(context, 'config') ? _.get(context, 'config') : _.get(context, 'book.config');
...
function renderUml(content) {
  ...
  var config = getConfig(this, 'pluginsConfig.plantuml-svg.config');
  if (config) {
    params.qs = { config: config };
    params.useQuerystring = true;
  }

  return (this.rpMock || rp).post(params).then(Base64.encode).then(OUTPUT_TEMPLATE);
}
